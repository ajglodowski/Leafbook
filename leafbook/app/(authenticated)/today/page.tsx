...
export default async function TodayPage() {
  ...
  const needsWater = dueTasks?.filter(
    (t) => t.watering_status === "overdue" || t.watering_status === "due_soon" || t.watering_status === "not_started"
  ) || [];
  
  const needsFertilizer = dueTasks?.filter(
    (t) => t.fertilizing_status === "overdue" || t.fertilizing_status === "due_soon"
  ) || [];
  ...
  // Count stats
  const overdueWaterCount = needsWater.filter(t => t.watering_status === "overdue").length;
  const dueSoonWaterCount = needsWater.filter(t => t.watering_status === "due_soon").length;
  const notStartedCount = needsWater.filter(t => t.watering_status === "not_started").length;
              ...
              <Card key={`water-${task.plant_id}`} size="sm">
                ...
                <CardContent className="flex items-center justify-between gap-2">
                  <p className="text-xs text-muted-foreground">
                    Last: {formatTimeAgo(task.last_watered_at)}
                  </p>
                  <CareButton 
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {needsFertilizer.map((task) => (
              <Card key={`fert-${task.plant_id}`} size="sm">
                ...
                <CardContent className="flex items-center justify-between gap-2">
                  <p className="text-xs text-muted-foreground">
                    Last: {formatTimeAgo(task.last_fertilized_at)}
                  </p>
                  <CareButton 
                    plantId={task.plant_id!} 
