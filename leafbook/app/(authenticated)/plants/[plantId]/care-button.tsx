...
import { Calendar, CalendarDays, Check, ChevronDown, Droplets, Sparkles, Sprout } from "lucide-react";
...
export function CareButton({ plantId, eventType, variant, status }: CareButtonProps) {
    ...
    <>
      <div className="flex">
        <Button
          ...
          className={`gap-1 rounded-r-none ${!isOverdue ? config.className : ""}`}
        >
          ...
          <DropdownMenuTrigger
            render={
              <Button
                size="sm"
                variant={isOverdue ? "destructive" : "default"}
                className={`px-1.5 rounded-l-none border-l border-white/20 ${!isOverdue ? config.className : ""}`}
                disabled={isPending}
              />
            }
          >
