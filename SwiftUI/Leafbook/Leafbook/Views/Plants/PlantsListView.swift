...
struct PlantsListView: View {
    ...
    private var headerSubtitle: String {
        ...
        let count = viewModel.plants.count
        guard count > 0 else { return base }
        return "\(base) · \(count) active plant\(count == 1 ? "" : "s")"
    }
    ...
    var body: some View {
            ...
            List {
                ...
                if let message = viewModel.errorMessage {
                    Text(message)
                        ...
                        .listRowBackground(LeafbookColors.background)
                } else if filteredPlants.isEmpty && !viewModel.isLoading {
        ...
        .navigationDestination(item: $selectedPlantId) { plantId in
